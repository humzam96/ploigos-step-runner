...
class Rekor(StepImplementer):  # pylint: disable=too-few-public-methods
    ...
    def create_rekor_entry( self,
        ...
        base64_encoded_artifact = base64.b64encode(json.dumps(content_dict).encode('utf-8')).decode('utf-8') #self.base64_encode(content_file)
        rekor_entry = {
            "kind": "rekord",
            "apiVersion": "0.0.1",
            "spec": {
    ...
    def get_signature(self, sig_file, image_tar_file):
    ...
    def upload_to_rekor(self, rekor_server, image_tar_file, content_dict):
        sig_file = image_tar_file + '.asc'
        sig_file_path = Path(sig_file)
        if sig_file_path.exists():
            sig_file_path.unlink()
        self.get_signature(sig_file,image_tar_file)
        rekor_entry = self.create_rekor_entry('/var/pgp-private-keys/gpg_public_key',sig_file, image_tar_file, content_dict)
